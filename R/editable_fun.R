...
deepstate_compile_generate_fun <- function(package_path,function_name,
  ...
  if(file.exists(fun_path)){
    ...
    range_elem <- nc::capture_all_str(harness_lines,"//",fun_call=".*","\\(")
                                          
    for(fun_range in range_elem$fun_call){
    ...
    deepstate_fuzz_fun(package_path,fun_name,sep="generation",verbose=verbose)
